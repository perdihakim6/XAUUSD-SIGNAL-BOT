...
{{"action":"BUY/SELL/WAIT","confidence":0-100,"entry":price,"sl":price,"tp":price,"rr":"1:X","trend":"BULLISH/BEARISH/NEUTRAL","dxy_aligned":true/false,"news_sentiment":"BULLISH/BEARISH/NEUTRAL","event_risk":"HIGH/MEDIUM/LOW","key_level":"nearest fib or SR level","reason":"max 50 words - combine ALL factors"}}"""
            ...
            if r.status_code == 200:
                ...
                raw_content = content  # keep original for debug logging

                if not content or not content.strip():
                    logger.error(f"Master AI error: empty content from model. Full response: {json.dumps(result)[:500]}")
                    return None
                ...
                # Fallback: extract the first {...} block in case the model added extra text
                ...
                try:
                    ...
                    logger.error(f"Master AI JSON parse failed: {je}. Raw content was: {raw_content[:500]!r}")
                    return None
